...
formValid.addEventListener('click', validation);
 ...
 emailvalid=/^[-a-z 0-9]+(\.[-a-z_éè~¨0-9])*@[a-z]\.[a-z]$/; 

 adrvalid=/^\d+(\s[-A-z0-9]+)+\s*$/;
...
function validation(event){
   ...
   //si le champ est vide
    ...
    oublinom.textContent="format incorrect ";
   ...
   if(pre.validity.valueMissing){
      ...
      oublipre.textContent="format incorect";
   ...
   oublipre.style.color="green";
    ...
    if(city.validity.valueMissing){
      ...
      oublicit.textContent="format incoreect ";
